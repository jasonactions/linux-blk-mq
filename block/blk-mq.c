 ...
 */
/*置位硬队列hctx->ctx_map的当前软队列所对应的bit*/
...
/*
 * free_tags中置位了空闲的request tag，因此free_tags置0位表示硬队列中正在处理的request，
 * 通过查询这些request的执行时间来判断是否超时，如果超时则执行超时处理
 * @free_tags: 包含了硬队列中所有request tag的bitmap,其中空闲的tag被置1
 */
static void blk_mq_timeout_check(void *__data, unsigned long *free_tags)
{
	...
	do {
		...
		/*查请求rq是否超时，如果超时则执行超时处理*/
		blk_rq_check_expired(rq, data->next, data->next_set);
	} while (1);
}

/*
 * 检查硬队列hctx中的每个在处理的request是已超时，
 * 如果已经超时，则执行超时处理，如果未超时则设定下次定时器的超时处理时间
 * @hctx: 硬队列
 * @next: 下次的定时器超时时间
 * @next_set: 是否已经更新了下次的定时器超时时间
 */
					...
					unsigned int *next_set)
{
	 ...
	 */
	/*
	 * 根据tag机制，遍历硬队列中每个正在处理的request，通过blk_mq_timeout_check检查是否超时，如果超时则执行超时处理
	 */
	blk_mq_tag_busy_iter(hctx->tags, blk_mq_timeout_check, &data);
}

/*此定时器是为了检测硬队列中的每个busy(正在处理)的request是否超时,如果超时则执行超时处理*/
 ...
 */
/*将与当前硬队列hctx有映射关系的软队列中的request分发给块设备驱动*/
static void __blk_mq_run_hw_queue(struct blk_mq_hw_ctx *hctx)
{
	...
	if (!list_empty_careful(&hctx->dispatch)) {
		spin_lock(&hctx->lock);
		/*为了保证公平，将重新入队的request放到rq_list的起始位置*/
	...
	if (!list_empty(&rq_list)) {
		spin_lock(&hctx->lock);
		/*hctx->dispatch用来存放由于设备busy重新入队的request*/
...
static void blk_mq_make_request(struct request_queue *q, struct bio *bio)
{
	...
	/*判断是否需要刷新cache，是否需要等待访问非易失存储器完成*/
	...
	/*从硬队列获取一个预先分配的request*/
	rq = __blk_mq_alloc_request(hctx, GFP_ATOMIC, false);
	if (likely(rq))
		/*初始化rq*/
		blk_mq_rq_ctx_init(ctx, rq, rw);
	/*如果没有空闲的request可用，则阻塞等待释放的request*/
	else {
		blk_mq_put_ctx(ctx);
			      ...
			      unsigned int reserved_tags, int node)
{
	...
	for (i = 0; i < hctx->queue_depth;) {
		...
		do {
			...
			if (page)
				break;
			/*如果分配不到如此大的页框块，则尝试分配小点的*/
			if (!this_order--)
				break;
			/*如果页框块大小小于一个请求的大小，就退出*/
	...
	/*分配tag,hctx->tags->free_tags为tag pool，i其中存放了tag, tag为per cpu变量，即每个cpu都会分配队列深度相同的tag*/
...
static void blk_mq_map_swqueue(struct request_queue *q)
{
	...
	queue_for_each_ctx(q, ctx, i) {
		...
		/*基于request.map数组获得*/
		hctx = q->mq_ops->map_queue(q, i);
		/*如果只有一个硬队列index_hw也不一致？*/
		ctx->index_hw = hctx->nr_ctx;
		hctx->ctxs[hctx->nr_ctx++] = ctx;
		pr_err("%s: hctx(%p), ctx(%p), q->nr_queues(%d),hctx->nr_ctx(%d),ctx->index_hw(%d),hctx->ctxs[%d](%p)\n",
				__func__, hctx, ctx, q->nr_queues, hctx->nr_ctx, ctx->index_hw, hctx->nr_ctx-1 ,hctx->ctxs[hctx->nr_ctx-1]);
	}
}
					...
					void *driver_data)
{
	...
	/*建立硬队列与软队列的映射关系,q->mq_map中保存了软队列映射的硬队列号*/
	q->mq_map = blk_mq_make_queue_map(reg);
	if (!q->mq_map)
		goto err_map;
	
	/*初始化请求队列(硬队列)请求处理超时定时器, 用以检查并处理请求超时*/
	...
	/*初始化泄洪work,处理函数为mq_flush_work，负责将请求从软队列调入硬队列*/
	blk_mq_init_flush(q);
	/*初始化软队列*/
