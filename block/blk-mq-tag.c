			  ...
			  void (*fn)(void *, unsigned long *), void *data)
{
	...
	/*
	 * 遍历硬队列tags->free_tags.tag_cpu->freelist, 如果freelist置位，说明对应的tag空闲，
	 * 通过__blk_mq_tag_iter置位tag_map对应位
	 * 此处说明tag_map可以作为一个收集器，收集了per cpu的tag_cpu的所有空闲的tag???
	 * 注：tag_cpu为per_cpu变量
	 */
...
/*
 * 分配tag，tags为tag pool, 分配的tag为per cpu变量,保存在tag pool中
 * @total_tags: 一般为队列深度
 */
