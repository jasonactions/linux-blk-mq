...
/*null队列的描述符，null队列用于存放nullb_cmd，且定义了队列深度*/
struct nullb_cmd {
...
static void null_cmd_end_ipi(struct nullb_cmd *cmd)
{
	...
	if (llist_add(&cmd->ll_list, &cq->list)) {
		...
		/*
		 *
		 * http://blog.chinaunix.net/uid-23769728-id-3523003.html
		 */
...
static void null_dump(void)
{
	int i;
	struct blk_mq_ctx *ctx;

	pr_err("null_mq_reg info...\n");
	pr_err("null_mq_reg.nr_hw_queues:%d\n", null_mq_reg.nr_hw_queues);
	pr_err("null_mq_reg.queue_depth:%d\n", null_mq_reg.queue_depth);
	pr_err("null_mq_reg.cmd_size:%d\n", null_mq_reg.cmd_size);
	pr_err("null_mq_reg.reserved_tags:%d\n", null_mq_reg.reserved_tags);
}
